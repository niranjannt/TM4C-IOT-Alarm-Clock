...
// write this
  ...
  SYSCTL_RCGCGPIO_R |= 2;    // 2) activate clock for Port B
  while((SYSCTL_PRGPIO_R&2) != 2){};  // 3 for stabilization
  GPIO_PORTB_DIR_R &= ~0x20;    // 4) make PE4 input
  GPIO_PORTB_AFSEL_R |= 0x20;   // 5) enable alternate function on PB4
  GPIO_PORTB_DEN_R &= ~0x20;    // 6) disable digital I/O on PB4
  GPIO_PORTB_AMSEL_R |= 0x20;   // 7) enable analog functionality on PB4
while((SYSCTL_PRADC_R&0x0001) != 0x0001){}; // good code, but not implemented in simulator
  ...
  ADC0_SSMUX3_R += 11;           // 12) set channel
 ...
 GPIO_PORTB_DATA_R = (GPIO_PORTB_DATA_R&(~31)|squareclock[alarmsoundtick]);   // output one value
    alarmsoundtick++;
     if(alarmsoundtick>=14){
		...
		if(tickflag){
				static uint32_t alarmsoundtick=0;
 GPIO_PORTB_DATA_R = (GPIO_PORTB_DATA_R&(~31)|ticks2[alarmsoundtick]);   // output one value
    alarmsoundtick++;
     if(alarmsoundtick>=2113){
        tickflag=0;
        alarmsoundtick=0;
     }
	 }
